...
public class MainConfig extends JFinalConfig {
	...
	public void configPlugin(Plugins me) {
		loadConfig();
		/*
		 * //配置数据库连接池插件 DruidPlugin dbPlugin=getDruidPlugin(); wallFilter = new
		 * WallFilter(); // 加强数据库安全 wallFilter.setDbType("mysql");
		 * dbPlugin.addFilter(wallFilter); dbPlugin.addFilter(new StatFilter());
		 * // 添加 StatFilter 才会有统计数据
		 * 
		 * //数据映射 配置ActiveRecord插件 ActiveRecordPlugin arp=new
		 * ActiveRecordPlugin(dbPlugin);
		 * arp.setShowSql(p.getBoolean("devMode")); arp.setDialect(new
		 * MysqlDialect()); dbPlugin.setDriverClass("com.mysql.jdbc.Driver");
		 *//******** 在此添加数据库 表-Model 映射 *********//*
													 * //如果使用了JFinal Model 生成器
													 * 生成了BaseModel 把下面注释解开即可
